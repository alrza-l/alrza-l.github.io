...
function Anthem_ExtractIFrameDocument(iFrameEl) {
    var doc = null;
    try {
        if (iFrameEl.contentWindow) // For IE5.5 and IE6
...
function Anthem_CallBack(url, target, id, method, args, clientCallBack, clientCallBackArg, includeControlValuesWithCallBack, updatePageAfterCallBack) {
    ...
    if (includeControlValuesWithCallBack) {
        if (typeof ($) == "undefined") //Checking JQuery Support
            var form = Anthem_GetForm();
        else
            var form = $(":input");
        if (form != null) {
                            ...
                            elementValue = element.value;
                                                ...
                                                if (textType == "CKEditor") {
                                                    //if ($(element).css("display") != "none") {
                                                        var editor_data;
                                                        eval("if (CKEDITOR.instances['" + element.id + "']) {editor_data = CKEDITOR.instances." + element.id + ".getData();}");
                                                        if (typeof (editor_data) != 'undefined')
                                                            elementValue = editor_data;
                                                    //}
                                                }
                                                else
                                                    elementValue = element.value;
    ...
    if (useIOFrame) {
        ...
        //ioframe.document.appendChild(theForm);
        ...
        for (i = 0; i < elementArray.length; i++) {
            var inputType = elementArray[i].id;
            //if (inputType != null)
            //if (inputType.substring(0,1) != "_")
